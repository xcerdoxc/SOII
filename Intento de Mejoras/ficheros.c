 ...
 * offset: posición de escritura inicial en bytes lógicos con respecto al ninodo
 * 
 * nbytes: número de bytes que vamos a leer
 *
...
{
    unsigned int primerBL, ultimoBL, desp1, desp2, ultimobyte, bytesLeidos = 0;
    ...
    else //Caso en que la operación de escritura afecte a más de un bloque
    {
        ...
        bytesLeidos = BLOCKSIZE - desp1; //Contabilizamos el número de bytes leidos
                                         //Bloques lógicos intermedios
        for (int i = primerBL + 1; i < ultimoBL; i++)
        {
        ...
        {
            bread(nbfisico, buf_bloque);
            memcpy(buf_original + bytesLeidos, buf_bloque, desp2 + 1); //Copiamos los bytes del bloque logico a buf_bloque
        }
        bytesLeidos += desp2 + 1; //Contabilizamos el número de bytes leidos
    }
    //Actualizamos la metainformación del inodo
    mi_waitSem();
    //Leemos el inodo actualizado
